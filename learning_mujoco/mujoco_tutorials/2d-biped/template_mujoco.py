...
def key_callback(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS or action == glfw.REPEAT:
        ...
        if key == glfw.KEY_W:  # Trigger the swing phase
            if fsm_hip == FSM_LEG2_SWING:
                fsm_hip = FSM_LEG1_SWING
            elif fsm_hip == FSM_LEG1_SWING:
                fsm_hip = FSM_LEG2_SWING
        elif key == glfw.KEY_A:  # Force retract leg 1
            fsm_knee1 = FSM_KNEE1_RETRACT
        elif key == glfw.KEY_D:  # Force retract leg 2
            fsm_knee2 = FSM_KNEE2_RETRACT

def update_fsm(pos_foot1, pos_foot2, abs_leg1, abs_leg2):
    global step_no
    global fsm_hip
    global fsm_knee1
    global fsm_knee2
...
def controller(model, data):
    ...
    # State Estimation
    quat_leg1 = data.xquat[1, :] #orientation (quat)
    euler_leg1 = quat2euler(quat_leg1)
    abs_leg1 = -euler_leg1[1]
    pos_foot1 = data.xpos[2, :] #position 
    ...
    # Transition check
    update_fsm(pos_foot1, pos_foot2, abs_leg1, abs_leg2)
