...
from src.utils.array_utils import ArrayType, inplace_update, loop_update
from src.utils.math_utils import quat2euler
from src.motion_generation.zmp_planner import ZMPPlanner
...
class ZMPWalk:
    ...
    def __init__(
        ...
        #In the future add a more dynamic way to do this. group joints by name (leg, arm, etc.) have user input the group names
        self.default_leg_joint_pos = self.default_joint_angles[7:]

        self.foot_to_com_x = float(robot.foot_to_com_x)
        self.foot_to_com_y = float(robot.foot_to_com_y)
        self.footstep_height = float(robot.footstep_height)
        self.com_z = float(robot.com_z)

        self.zmp_planner = ZMPPlanner()

    def build_lookup_table(self, command_range = [[-0.2, 0.4], [-0.2, 0.2], [-0.8, 0.8]],
        interval: float = 0.02,):
    ...
    def plan(
        ...
        # )()
        #Durations of different walking cycles
        time_list = np.array(
